...
const $body = $('body');
const $section = $('<section id="middle-earth">Middle Earth');
...
const $ul = $('<ul/>').attr("id", "pal");
$('aside').append($ul);
  // 3. give each of the buddies a class of "buddy"
for(let i=0; i < buddies.length; i++) {
  const $li = $("<li/>").text(buddies[i]).attr("class", "buddy");
  $('#pal').append($li);
  };
};
...
window.alert("The Horn of Gondor has been blow");
  // 2. Boromir's been killed by the Uruk-hai! Put a linethrough on Boromir's name
$(".buddy:contains('Boromir'):first").addClass("boromir");

  // 3. Tricky: Remove the Uruk-Hai from the Baddies on the page
  $(".baddy:contains('Uruk'):first").remove();
};
...
$("<div id=\"the-ring\"></div>").appendTo('#gollum');
